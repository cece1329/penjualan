...
class ReceiptPdfPrinter(private val context: Context) {
    ...
    fun printToBluetooth(receipt: ReceiptData) {
        ...
        if (pairedDevices.isNullOrEmpty()) {
            Toast.makeText(context, "Pasangkan printer Bluetooth Anda terlebih dahulu", Toast.LENGTH_LONG).show()
            return
        }
        ...
        Thread {
            ...
            try {
                ...
                (context as? android.app.Activity)?.runOnUiThread {
                    Toast.makeText(context, "Berhasil mencetak ke ${device.name}", Toast.LENGTH_SHORT).show()
                    // Update printed status in Firebase if possible
                    if (!receipt.idTransaksi.isNullOrBlank()) {
                        com.google.firebase.database.FirebaseDatabase.getInstance()
                            .getReference("transaksi").child(receipt.idTransaksi).child("isPrinted").setValue(true)
                    }
                }
            } catch (e: Exception) {
                (context as? android.app.Activity)?.runOnUiThread {
    ...
    internal fun drawReceipt(canvas: Canvas, receipt: ReceiptData, targetWidth: Float = BASE_W) {
        ...
        receipt.items.forEach { item ->
            ...
            p.textSize = 8.5f
            p.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
            canvas.drawText("  ${item.namaProduk}", ml + 4f, y + 10f, p)
            ...
            p.textSize = 10f
            p.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
            canvas.drawText("${item.jumlah}", cx, y + 15f, p)

            p.textAlign = Paint.Align.RIGHT
            p.color = 0xFF4A2B66.toInt()
            p.textSize = 9f
            canvas.drawText("Rp ${fmt(item.jumlah * item.harga)}  ", mr - 2f, y + 15f, p)
            y += 34f
        }
        ...
        p.color = 0xFF4A2B66.toInt()
        canvas.drawRect(0f, y + 15f, W, y + 25f, p) 
    ...
    fun buildTextReceipt(receipt: ReceiptData): String {
        ...
        receipt.items.forEach { item ->
            val nameText = if (item.namaProduk.length > 20) item.namaProduk.take(17) + "..." else item.namaProduk
            sb.append("$nameText\n")
            val subtotal = "Rp ${fmt(item.jumlah * item.harga)}"
            sb.append(leftRight("  ${item.jumlah} x ${fmt(item.harga)}", subtotal))
        }
    ...
    private fun drawReportDoc(canvas: Canvas, report: ReportSummary) {
        ...
        canvas.drawRect(margin, y, width - margin, y + 1.5f, p) 
